...
import { NativeSlot } from '@tarojs/components';
...
interface EnterProps {
  ...
  redirect?: EnterRedirectBase; // 进入什么页面，默认技能列表，进入技能详情页请传 {name:'chat',params:{microAppId:'xxx'}}
  ...
  onLisenReturn: (data: any) => { microAppId: string; data: any }; // 监听返回值
}
...
const Enter: React.FC<EnterProps> = ({
  ...
  const handeClick = () => {
    ...
    if (redirect && redirect.name === 'chat' && redirect.params?.microAppId) {
      let paramsJSON = redirect.params ? '&params=' + encodeURIComponent(JSON.stringify(redirect.params)) : '';
      onNavigate &&
        onNavigate({
          url: `/plugin/pages/chat?microAppId=${redirect.params?.microAppId}${paramsJSON}`,
          events: {
            Return: (data) => {
